"""Parser para extrato do Santander (CSV, PDF, OFX)."""
...
class SantanderParser(AbstractParser):
    bank_name = "santander"
    ...
    def _parse_csv(self, path: Path) -> list[TransactionRaw]:
        ...
        desc_col = next((c for c in df.columns if any(k in c.lower() for k in ["descr", "hist", "lanc"])), None)
    ...
    def _parse_pdf(self, path: Path) -> list[TransactionRaw]:
        ...
        with pdfplumber.open(path) as pdf:
            for page in pdf.pages:
                ...
                for table in tables:
                    results.extend(_extract_from_table(table, "santander", str(path)))
                if not tables:
                    text = page.extract_text() or ""
                    results.extend(_extract_from_text_lines(text, "santander", str(path)))
        return results
